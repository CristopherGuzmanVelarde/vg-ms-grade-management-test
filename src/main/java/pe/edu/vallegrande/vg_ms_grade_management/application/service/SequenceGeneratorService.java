...
public class SequenceGeneratorService {
    ...
    public Mono<String> generateSequence(String seqName) {
        ...
        return mongoOperations.findAndModify(query, update, options, DatabaseSequence.class)
                ...
                .defaultIfEmpty("1") // In case the sequence doesn't exist yet, start with 1
                .flatMap(seq -> {
                    // Ensure the first value is 1 if it was newly created and defaulted to 1L
                    // or if the existing sequence was somehow 0 or less.
                    if (seq.equals("0")) {
                        DatabaseSequence newSeq = new DatabaseSequence(seqName, "1");
                        return mongoOperations.save(newSeq).thenReturn("1");
                    }
